...
$$ h\_{t+1} - h_t \approx \Delta (\mathbf{A}h_t + \mathbf{B}x_t) $$

$$ \Rightarrow h\_{t+1} \approx (I + \Delta A)h_t + (\Delta \mathbf{B})x_t $$
...
In regular SSMs, A, B, C and D are learned matrices - that is $$ \mathbf{A} =
\mathbf{A}\_\theta$$ etc. (where theta represents the learned parameters)
