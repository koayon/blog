...
We let $$ h_t = h(t) \ \text{and} \ h\_{t+1} = h(t + \\Delta) $$ and substitute
into Equation 1a giving:

$$ h\_{t+1} - h_t \approx \Delta (\mathbf{A}h_t + \mathbf{B}x_t) $$

$$
\Rightarrow h\_{t+1} \approx (I + \Delta \mathbf{A})h_t + (\Delta
\mathbf{B})x_t
$$
...
\mathbf{A}\_\theta$$ etc. (where θ represents the learned parameters)
